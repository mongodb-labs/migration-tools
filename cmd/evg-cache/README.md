...
When generating the cache hit expansion name, any dashes in `<cache-name>` will be converted to
underscores.
...
git commit -m "Generate evg-cache YAML for <cache-name>"
```
...
If the cache changes a lot or you have a very large number of tasks that use the same cache, a warm
up task is probably a net speed up. If the cache changes infrequently and only a few tasks use it,
then the warm up may be a net slow down.

## Multiple caches

When a project uses more than one cache, the `set-up-evg-cache-scripts` function only needs to
appear in one generated file — Evergreen rejects duplicate function names at config-load time.

Generate a primary file (with setup) and one or more secondary files (without):

```sh
evg-cache generate \
  --name mise-and-go \
  --bucket mciuploads \
  --namespace myproject/mise-cache \
  --key-file mise.toml \
...
evg-cache generate \
  --name go-modules \
  --bucket mciuploads \
  --namespace myproject/go-cache \
  --key-file go.sum \
  --cache-path .gomodcache \
  --omit-setup-function \
  --output-file evergreen/functions/go-modules-cache.yml
```

Each file is a self-contained YAML document with a `functions:` key. Include both in your Evergreen
config:

```yaml
include:
  - filename: evergreen/functions/mise-and-go-cache.yml
  - filename: evergreen/functions/go-modules-cache.yml
```
...
| `--omit-setup-function` |                | false          | Omit `set-up-evg-cache-scripts` from output; use when generating a secondary cache file that will be included alongside a primary file that already defines the shared setup function (see [Multiple caches](#multiple-caches)) |
