...
// Use NewBuilder to construct. All script paths use the ScriptDir field.
type Config struct {
	Bucket            string
	Namespace         string
	Name              string
...
func NewBuilder(name string) (*Builder, error) {
	...
	return &Builder{config: Config{Name: name, ScriptDir: "./evg-cache-scripts"}}, nil
}
...
// WithScriptDir sets the directory for runtime scripts (relative to ${workdir}).
// Defaults to ./evg-cache-scripts if not set.
func (b *Builder) WithScriptDir(dir string) *Builder {
	b.config.ScriptDir = dir
	return b
}
...
func (b *Builder) Build() Config {
	...
	case b.config.ScriptDir == "":
		panic("cache.Builder: ScriptDir must not be empty")
	}
...
func (c *Config) scriptPath(script string) string {
	return strings.TrimRight(c.ScriptDir, "/") + "/" + script
}
