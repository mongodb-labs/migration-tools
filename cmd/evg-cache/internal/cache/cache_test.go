...
func (s *CacheTestSuite) TestBuild_ScriptDirIsAlwaysEvgCacheScripts() {
	b, err := cache.NewBuilder("foo")
	s.Require().NoError(err)
	cfg := b.WithBucket("b").
		WithNamespace("ns").
	...
	s.Assert().Equal(
		"./evg-cache-scripts",
		cfg.ScriptDir,
		"ScriptDir should always be ./evg-cache-scripts",
	)
}
...
func (s *CacheTestSuite) TestDisplayName_WithDisplayNameOverridesDefault() {
	...
	y := s.mustMarshal(cfg.SaveCommands())
	s.Assert().Contains(y, "mise and go executables",
		"s3.put display_name should reflect the configured DisplayName")
}
